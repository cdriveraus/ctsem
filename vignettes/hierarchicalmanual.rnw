...
The function \verb}ctStanContinuousPars} can be used to return the population level continuous time parameter matrices in matrix form. Here we return the 97.5 quantile for each of the elements in the matrices:
