...
\usepackage[natbibapa]{apacite} % Use natbib with APA style
\bibliographystyle{apacite}     % Set bibliography style
% \usepackage{natbib}
...
% Redefine jss commands to do nothing
\newcommand{\pkg}[1]{#1}
...
MANIFESTVAR & $\Theta$ & free & typically diagonal matrix of var / cov between manifests, specified with standard deviations on diagonal and zeroes elsewhere.\\
...
<<fitting,include=FALSE>>=
suppressWarnings(fit<-ctFit(datalong = ctstantestdat,
  model = model,optimize=TRUE,cores=1,
  # savescores=TRUE,savesubjectmatrices = TRUE, 
  priors=TRUE))
@

<<fittingshow,include=TRUE,eval=FALSE>>=
fit<-ctFit(datalong = ctstantestdat, model = model, priors=TRUE)
@
...
The function \verb}ctSummaryMatrices} can be used to return the population level parameter matrices in matrix form. Here we return the 97.5 quantile for each of the elements in the matrices:

<<ctSummaryMatrices,eval=FALSE>>=
ctSummaryMatrices(fit,calcfunc = quantile, calcfuncargs = list(probs=.975))
@
...
<<plots1,fig.width=5, fig.height=3,fig.cap='Discrete-time cross-effect dynamics of the estimated system for a range of time intervals, with 95\\% credible intervals.'>>=
try({
  ctDiscretePars(fit, plot=TRUE, indices = 'CR')
})
@

Figure \ref{fig:plots1} shows the dynamic regression coefficients (between latent states at different time points) that are implied by the model for particular time intervals, as well as the uncertainty (default is 95\% credible interval) of these coefficients. These are in the processes' own units, which for the default \verb|impulseType='unit'| is simply \verb|expm(DRIFT*t)|; \verb|standardise=TRUE| instead gives the change in the outcome process' standard deviations per one standard deviation change in the predicting process, which is comparable across processes on different scales. In this case the estimates are of the cross regression effects, obtained by sampling from all subjects data, but specific subjects, as well as specific indices of the effects (e.g., indices = 'AR' or indices = rbind(c(2,1))' can be specified. The mapping of subject id to the internal sequential integer representation can be found via \verb|fit$setup$idmap| . 
...
ctPlotPosterior(obj = fit, rows=3)
@
...
ctPredict(fit, subjects=c(2,4,5),  kalmanvec=c('y', 'yprior'),
  plot=TRUE, timestep=.01)
@

\subsection{Time independent predictor effect plots}

Because time independent predictors give a linear effect \textit{prior} to any necessary transformations, the effects necessarily become non-linear when applied to bounded parameters, which can make them difficult to conceptualise. To aid with this, a visual summary of the full range of effects can be seen using the \verb|ctTIpredEffects| function, as follows:

<<tipredeffects, fig.width=5, fig.height=4, fig.cap='Expectations for individuals parameter values change depending on their score on time independent predictors.'>>=
ctTIpredEffects(fit, plot = TRUE, whichpars=c('dtDRIFT[2,1]','CINT'),
...
The same elements can instead be given by name, in any order and without counting separators. A cell is read as a named specification when it names one of the fields (\verb|transform|, \verb|indvarying|, \verb|sdscale|, \verb|tipreds|) and assigns to it with \verb|=|; as an ordered sequence when it contains the \verb{|{ character; and as a plain parameter name otherwise. Fields are separated by commas, and a comma inside brackets belongs to the value, so \verb|transform=pnorm(param, 0, 1)| is a single field. \verb|ctParSpec()| writes the ordered form from the same named arguments, which is useful when a specification is built programmatically. The three DRIFT[1,1] specifications shown below are equivalent.
...
#Change during model specification -- ordered sequence
model<-ctModel(type='ct',
...
#the same, with the fields named
model<-ctModel(type='ct',
  DRIFT = matrix(c('drift_eta1_eta1, transform=2 * param -1, indvarying=TRUE',
    'dr21','dr12','dr22'),2,2),
  latentNames=c('eta1','eta2'),
  manifestNames=c('Y1','Y2'),
  TDpredNames='TD1',
...
#the same again, written by ctParSpec
model<-ctModel(type='ct',
  DRIFT = matrix(c(
    ctParSpec('drift_eta1_eta1', transform='2 * param -1', indvarying=TRUE),
    'dr21','dr12','dr22'),2,2),
  latentNames=c('eta1','eta2'),
  manifestNames=c('Y1','Y2'),
  TDpredNames='TD1',
...
An alternative approach to specifying the tipredeffects is in the initial model specification, using either the `|' separator notation or the named form. The following shows an example where the DRIFT[1,1] parameter is influenced by covariates TI1 and TI3,  DRIFT[1,2] is influenced by TI2, and there are no other time independent predictor effects (because \verb|tipredDefault=FALSE|). The two model specifications are equivalent -- with separators, the predictors are the fifth field, so four separators are needed to reach it; named, they are simply \verb|tipreds=|.
<<echo=TRUE,eval=FALSE,tipredspec>>=
model<-ctModel(type='ct', tipredDefault = FALSE,
  DRIFT = matrix(c(
...
model<-ctModel(type='ct', tipredDefault = FALSE,
  DRIFT = matrix(c(
    'drift_eta1_eta1, tipreds=c(TI1, TI3)','dr21, tipreds=TI2',
    'dr12','dr22'),2,2),
  latentNames=c('eta1','eta2'),
  manifestNames=c('Y1','Y2'),
  TDpredNames='TD1',
...
% The first approach is to give free parameters of the T0VAR or T0MEANS matrix the name \verb{'stationary'{, which can be useful if for instance only the initial variance should be stationary, or just one of the initial variances. Another approach is to set the argument stationary=TRUE to ctFit. Specifying this argument then ignores any T0VAR and T0MEANS matrices in the input, instead replacing them with asymptotic expectations based on the DRIFT, DIFFUSION, and CINT matrices. When stationarity is requested and nonlinearities are specified in the model, the computations are based on the starting DRIFT, DIFFUSION, and CINT matrices.

\subsection{Accessing Stan model code}
For diagnosing problems or modifying the model in ways not achievable via the ctsem model specification, one can use ctsem to generate the Stan code and then work directly with that, simply by specifying the argument \verb|fit=FALSE| to the \verb|ctFit| function, and accessing the \verb|$stanmodeltext| subobject. Any altered code can be passed back into \verb|ctFit| by using the \verb|stanmodeltext| argument, which can be convenient for setting up the data in particular.

\subsection{Using Rstan functions}
The standard rstan output functions such as summary and extract are also available whenever Stan's sampler is used, and the shinystan package provides an excellent browser based interface. The stan fit object is stored under the \verb|$stanfit| subobject of the \verb|ctFit| output. The parameters which are likely to be of most interest in the output are prefixed by \verb|pop_| for pop (population) mean, and \verb|popsd| for pop standard deviation. Any \verb|pop| parameters are returned in the form of the continuous time matrix equations (or discrete time when estimated as such). Subject specific parameters are denoted by the matrix they are from, then the first index represents the subject id, followed by standard matrix notation. For example, the 2nd row and 1st column of the DRIFT matrix for subject 8 is \verb|DRIFT[8,2,1]|. Parameters in such matrices are returned in the form used for internal calculations -- that is, variance covariance matrices are returned as such, rather than the lower-triangular standard deviation and correlation matrices required for input.
