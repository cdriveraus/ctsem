...
The code above specifies a first order bivariate latent process model, with each process measured by a single, potentially noisy, manifest variable. A single time dependent predictor is included in the model, and a time independent predictor. To see the subject level model equation, the \verb|ctModelLatex| function can be used on the model object, generating output as shown in Figure \ref{fig:ctmodellatexout}.
...
<<kalmanplot, fig.width=8, fig.height=5, fig.cap='Predictions for one subject with two processes. Uncertainty shown is a 95\\% credible interval comprising both process and measurement error.'>>=
ctKalman(fit, subjects=2,  kalmanvec=c('y', 'etaprior'), 
...
<<nl,eval=TRUE,messages=FALSE,cache=TRUE, fig.width=8, fig.height=4,fig.cap='Nonlinear oscillation example'>>=
sunspots<-sunspot.year
sunspots<-sunspots[50: (length(sunspots) - (1988-1924))]
id <- 1
time <- 1749:1924
...
m <- ctModel(n.latent=2, n.manifest=1, type='stanct', manifestNames = 'sunspots',
  LAMBDA=matrix(c(1,'ma1|log(1+exp(param))'),nrow=1, ncol=2),
  DRIFT = matrix(c(0,'drift21 | -log1p(exp(param))',1,  
  '-log1p(exp(PARS[1,2] + PARS[1,1] *  state[1]))'), 2, 2),
  DIFFUSION = matrix(c(0,0, 0,'diff11'), 2, 2),
  MANIFESTVAR=matrix(0),
  MANIFESTMEANS=matrix(c('m1|param * 10 + 44'), nrow=1, ncol=1),
  PARS=matrix(c('driftbystate','driftintercept'), 1,2) )

ssfitnl <- ctStanFit(datalong, m, optimize=TRUE)

k <- ctStanKalman(ssfitnl) #extract state predictions

matplot(datalong$time,t(k$yprior[1:50,,1]),type='l',
...
One important difference when using nonlinear estimation, is that DIFFUSION and T0VAR matrices are treated simply as Cholesky factors of covariance matrices -- this simplifies usage but may require more care with respect to prior specification / checking.
