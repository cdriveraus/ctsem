...
The function \verb}ctContinuousPars} can be used to return the population level parameter matrices in matrix form. Here we return the 97.5 quantile for each of the elements in the matrices:

<<ctContinuousPars,eval=FALSE>>=
ctContinuousPars(fit,calcfunc = quantile, calcfuncargs = list(probs=.975))
@
