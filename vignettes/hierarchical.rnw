...
long[,'time']=long[,'time']+round(rnorm(nrow(long),0,.1),2)
long[c(1:5,87:89),]
@
...
datalong <- cbind(id, time, sunspots)
 ...
 model$parameters$indvarying<-FALSE #Because single subject
 model$parameters$transform[14]<- '(param)*5+44 ' #Because not mean centered
 model$parameters$transform[7]<-'-log(exp(-param*1.5)+1)' #Higher order cross effect
 model$stationaryvarprior <- 10 
 model$stationarymeanprior <- 10

fit <- ctStanFit(datalong, model, iter=200, chains=2, 
initwithoptim=TRUE)
...
pred<-ctStanPredictions(ctstanmodelobj=model, 
ctstanfitobj=fit, subjects=1, datalong=datalong)
