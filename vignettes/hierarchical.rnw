...
m <- ctModel(n.latent=2, n.manifest=1, type='stanct', manifestNames = 'sunspots',
  LAMBDA=matrix(c('1 + PARS[1,3] * (state[1] + PARS[1,4])',0),nrow=1, ncol=2),
  DRIFT = matrix(c(0, '1|-2|drift21', 'PARS[1,2] + PARS[1,1] *  (state[1] + PARS[1,4])', 'drift22'), 2, 2),
  DIFFUSION = matrix(c('diff11',0,0, 0), 2, 2),
  MANIFESTVAR=diag(.01,1),
  PARS=matrix(c('driftcrossbystate','1|driftcrossintercept','lambdabystate','state1adjust'), 1,4) ) #
sink('../testout.txt')
ssfitnl <- ctStanFit(datalong, m, iter=300, chains=2, optimize=T,verbose=3)
sink()
ssfitnl <- ctStanGenerateFromFit(ssfit,100,F)
e=extract(ssfitnl)
...
k=ctStanKalman(ssfitnl)

dim(k$ypred)
plot(datalong[,'sunspots'],type='l',col='red')
matplot((k$ypred[1,,1]),type='l',ylim=c(-5,60),col=1,add=T)
matplot((k$err[1,,1]),type='l',col=1)
matplot(t(k$etaprior[1:50,,2]),type='l',col=1)

matplot(ssfitnl$generated$Y[,1:5,1],ylim=c(-10,200),type='l')


@

In this example we also made use of a faster form of parameter specification, in which parameter labels can be preceded by a sequence of numbers followed by \verb{|{ symbols as a separator, representing how the parameter should be transformed from an unbounded standard normal. The separate numbers specify the elements of the \verb|pars| subobject of the model, i.e. transform, multiplier, meanscale, offset, inneroffset. So the \verb{'1|ma1'{ label specifies a softplus ( \code|log(1+exp(param)| ) transform for the ma1 parameter, with defaults used for latter arguments when only some are specified.
