...
<<sunspotseval,echo=FALSE,cache=TRUE>>=
 sunspots<-sunspot.year
 sunspots<-sunspots[50: (length(sunspots)-(1988-1924))]
 id <- 1
 time <- 1749:1924
datalong <- cbind(id,time,sunspots)
 ...
 model <- ctModel(type='stanct', n.latent=2, n.manifest=1, 
  ...
  latentNames=c('sunspots_level','sunspots_velocity'),
   ...
   MANIFESTMEANS=matrix(c('m1'), nrow=1, ncol=1),
   MANIFESTVAR=matrix(0.0001, nrow=1, ncol=1),
   CINT=matrix(c(0, 0), nrow=2, ncol=1),
   DIFFUSION=matrix(c(
     0, 0,
     0, "diffusion"), ncol=2, nrow=2, byrow=TRUE))
 ...
 model$parameters$indvarying<-FALSE
 model$parameters$transform[14]<- '(param) *5+44 '
 model$parameters$transform[7]<-'-exp(param)'
 model$stationarymeanprior<-5
 model$stationaryvarprior<-5

fit <- ctStanFit(datalong, model, plot=FALSE, iter=20, chains=3,initwithoptim=T)
...
grid()
plot(density(2/(1+exp((param)*-1.5))-1,n=5000,bw=.03,from=-1,to=1), type='l', lwd=3, xlim=c(-1,1), xaxs='i', ylim=c(0,.6), yaxs='i', xlab='Value',main='Correlation')
grid()
