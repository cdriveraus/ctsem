...
Once model specification is complete, the model is fit to the data using the \verb|ctStanFit| function as shown in the following example. Depending on the data, model, and number of iterations requested, this can take anywhere from a few minutes to days. Current experience suggests 300 iterations is often enough to get an idea of what is going on, but more may be necessary for robust inference, but this is highly dependent on the specifics. For the sake of speed for this example we only sample for 200 iterations. With these settings the fit should take only a minute or two (but may not be adequate for inference!). Those that wish to try out the functions without waiting, can simply use the already existing \verb|ctstantestfit| object instead of creating the fit object (the code for this is commented out). The dataset specified here is built-in to the ctsem package, and available whenever ctsem is loaded in R. 
...
\hlstd{fit}\hlkwb{<-}\hlkwd{ctStanFit}\hlstd{(}\hlkwc{datalong} \hlstd{= ctstantestdat,} \hlkwc{ctstanmodel} \hlstd{= model,} \hlkwc{iter}\hlstd{=}\hlnum{200}\hlstd{,}
  \hlkwc{chains}\hlstd{=}\hlnum{2}\hlstd{,} \hlkwc{plot}\hlstd{=}\hlnum{FALSE}\hlstd{)}
...
\hlkwc{plot}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{plotcontrol}\hlstd{=}\hlkwd{list}\hlstd{(} \hlkwc{xaxs} \hlstd{=} \hlstr{"i"}\hlstd{,} \hlkwc{xlim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{16}\hlstd{),} \hlkwc{main} \hlstd{=} \hlstr{'Predicted'}\hlstd{))}
...
\hlkwc{plot}\hlstd{=}\hlnum{TRUE}\hlstd{,} \hlkwc{plotcontrol}\hlstd{=}\hlkwd{list}\hlstd{(} \hlkwc{xaxs} \hlstd{=} \hlstr{"i"}\hlstd{,} \hlkwc{xlim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{16}\hlstd{),} \hlkwc{main} \hlstd{=} \hlstr{'Smoothed'}\hlstd{))}
\end{alltt}
\end{kframe}\begin{figure}[htbp]
...
Because time independent predictors give a linear effect \textit{prior} to any necessary transformations, the effects necessarily become non-linear when applied to bounded parameters, which can make them difficult to conceptualise. To aid with this, a visual summary of the full range of effects seen in the data and model combination, can be seen using the \verb|ctStanTIpredeffects| function, as follows:
...
\hlkwd{ctStanTIpredeffects}\hlstd{(fit,} \hlkwc{plot} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{whichpars}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{'dtDRIFT'}\hlstd{,}\hlstr{'MANIFESTVAR[2,2]'}\hlstd{),}
 \hlkwc{timeinterval} \hlstd{=} \hlnum{.5}\hlstd{,} \hlkwc{whichTIpreds} \hlstd{=} \hlnum{3}\hlstd{,} \hlkwc{includeMeanUncertainty} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}
\end{kframe}\begin{figure}[htbp]
