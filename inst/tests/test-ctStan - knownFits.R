...
test_that("time calc", {
  ...
  AnomAuthfit<-ctFit(AnomAuth, AnomAuthmodel,stationary='T0VAR')
  ...
  AnomAuthstanmodel<-ctModel(type='stanct',LAMBDA=matrix(c(1, 0, 0, 1), nrow=2, ncol=2),  
    n.latent=2,n.manifest=2, 
    MANIFESTVAR=diag(0.01,2),
    Tpoints=5)
  AnomAuthstanmodel$parameters$indvarying<-FALSE
  sfit<-ctStanFit(datalong=dl,ctstanmodel=AnomAuthstanmodel,optimize=F,noncentered=F,
    iter=500,chains=2,plot=T,stationaryvar=F,fit=T,kalman=F)
