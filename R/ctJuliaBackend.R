...
.ctJuliaLaplaceInner <- function(laplacecontrol) {
  ...
  if (!is.null(floor)) {
    ...
    if (is.na(floor) || !floor %in% c("total", "gated")) {
...
.ctJuliaFitLaplaceBlock <- function(model_spec, result) {
  ...
  list(
    ...
    # The floor the objective was built with; post-fit routes rebuild the
    # objective from the same specification, so they use it too.
    floor = as.character(.ctJuliaOr(model_spec$laplace$inner$floor, "total")),
    gated_units = if (is.null(result$gated_units)) NA_integer_ else
      as.integer(result$gated_units),
...
.ctFitJuliaBackendImpl <- function(datalong, model, prepared_data = NULL,
  ...
  # in `.ctJuliaUnsupported()`). `'none'` is not a reachable value here --
  # it only ever arises on the `optimize=FALSE` route, which returns before
  # this line -- so `'laplace'` is the one value worth naming.
  ...
  optim_run <- list(
    ...
    # And whether it ended because it had stopped making progress with a
    # transform flat as the reason. Reported separately from `stopped_by_gap`
