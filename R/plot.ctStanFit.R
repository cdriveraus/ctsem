...
#'  \code{\link{ctStanDiscretePars}},\code{\link{ctStanKalman}},
#'  \code{\link{ctStanPlotPost}},\code{\link[rstan]{stan_trace}},
#'  \code{\link[rstan]{stan_dens}},\code{\link[rstan]{stan_plot}}
...
plot.ctStanFit <- function(x, types='all',wait=TRUE,...){
  ...
  if('kalman' %in% types && continue){
    message('Plotting expectations from Kalman filter using ctStanKalman')
    
    ctStanKalman(x, plot=TRUE,...)
