...
transformed parameters{
  ...
  ',if(any(indvarying)) paste0('
    hypercovchol=diag_pre_multiply(exp(loghypersd) .* sdscale, hypercorrchol);
  ...
  ',paste0(unlist(lapply(1:nrow(ctspec),function(rowi){
    if(ctspec$indvarying[rowi]) paste0('output_hsd_',ctspec$param[rowi],' = ',
      ...
      if(!is.na(ctspec$transform[rowi])) paste0(
        ...
        ((', 
        gsub('param', paste0('(hypermeans[',which(ctspec$param[is.na(ctspec$value)] == ctspec$param[rowi]),'] + .01 * output_hsd_',
...
if(fit==TRUE){
  ...
  if(optimize==TRUE && fit==TRUE) {
    ...
    out <- rstan::optimizing(object = out@stanmodel, 
      # init=0,
      # algorithm='BFGS',
      as_vector=F,
      history_size=100,
  ...
  if(vb==TRUE && fit==TRUE) {
    out <- rstan::vb(object = out@stanmodel, 
      iter=iter,
      # eta=1e-6,
      data = standata,...)
