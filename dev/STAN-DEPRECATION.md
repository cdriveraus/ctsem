...
| `ctFitCheck()`, `ctFitMelt()`, `R/ctFitCovCheck.R:558-998` | stan-only generated-versus-observed diagnostic, about 440 lines including large commented-out former features | the code; `ctFitCheckCov()` and `ctPostPredPlots()` cover the purpose on both backends | `ctFitCheck` is exported, so it needs a deprecation message pointing at `ctFitCheckCov` | J1.5 F4, F13 |
