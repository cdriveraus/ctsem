...
test_that("anomauth", {
 ...
 if( .Machine$sizeof.pointer != 4){
  ...
  sf=ctStanFit(ctDeintervalise(ctWideToLong(AnomAuth,Tpoints = AnomAuthmodel$Tpoints,n.manifest = 2)),
    ctstanmodel = sm, optimize=TRUE,verbose=1,savescores = FALSE,cores=6,nopriors=TRUE,
    optimcontrol=list(finishsamples=100,stochastic=T),plot=10,nlcontrol=list(nldynamics=FALSE))
