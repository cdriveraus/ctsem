...
system.time({f <- ctFit(datalong = d,model= m,cores=2,priors = F)})
...
f2 <- ctFit(datalong = d,model= m,cores=6,optimize=F,chains=4,intoverpop = F)
s2=summary(f2)
f3 <- ctFit(datalong = d,model= m,cores=6,optimize=F,chains=4,intoverpop = F,intoverstates = F)
s3=summary(f3)
}
