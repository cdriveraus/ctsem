...
- `ctGenerate(backend='julia')` generates individually varying parameters and time independent predictor effects, where it previously refused a model that asked for them. A population standard deviation is stated as `indvaryingsd` on the parameter, a predictor effect as `<TI>_effectsize`, and a whole population covariance through the new `POPCOV` matrix; anything left unstated falls back to the centre of the prior the fit would have used, so two generations from one specification agree.
