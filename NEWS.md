...
- Fixed: the covariance and correlation heatmaps of `ctFitCheck()` (`ctCheckFit()`) produced no output on their default settings, erroring with `corr=TRUE` and returning silently empty with `corr=FALSE`.
