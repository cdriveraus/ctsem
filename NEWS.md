...
## 27/8/2026
### 3.12.0 (development)
- A step size that was restarted and then never updated became 1, and sampling used it. The dual averaging reports `exp(logepsbar)` as the step size to sample at, and restarting the averaging -- which happens whenever the metric changes -- set `logepsbar` to zero, meaning a step size of 1. One update erases that, because the first update's weight is 1, so the fault only appears when a restart is the last thing warmup does. Two routes reach it and both are live: the divergence-triggered acceptance raise checks on a fixed stride of 50, so on a 500-iteration warmup it fires on iteration 500 with no iteration left to update the average; and `nwarmup = 0` never updates at all. The chain then samples at 1 on a target whose adapted step size is around 0.03, diverges on every transition, and returns draws from a stuck chain -- one such chain among four took R-hat to 4.11 and the minimum effective size to 2.2 across 2000 draws, with all 500 divergences its own. `logepsbar` now starts at `log(eps)` in both the constructor and the restart, which is a no-op anywhere an update follows. This also explains why `nwarmup = 0` had appeared to be useless: the initial step size was not going unadapted, it was being discarded and replaced by 1.
- `ctSample()` accepted `minEss`, `meanEss`, `maxDraws` and `rhatTarget`, documented them, and did not pass them to the sampler, so asking it to draw towards an effective sample size did nothing. `ctSample()` and `ctFit(optimize = FALSE)` had each built their own argument list for the same engine entry and the two had drifted over five settings, in both directions -- `maxdelta`, `init_scale` and `adapt_metric` went the other way, and two knobs had Stan's names on one path and the engine's on the other. There is now one call site and one place a control list is read, accepting both spellings, so nothing that worked stops working.
- Multi-chain fits reported no random effects. The separate-process path, which became the default for more than one chain in this version, was a third copy of the result-assembly tail, and four things had gone missing from it: the diagnostics carried no class, so `print()` printed a list; `$start` was absent; the constrained draws still described the optimised fit rather than the samples; and the effect summaries were dropped entirely. It now builds the engine's own result shape and goes through the same assembler as the in-process path.
- `ctFit(optimize = FALSE)` had no process-parallel chains at all, because the worker's entry point was `ctSample()` itself, which fixes the joint target and refuses any fit without a Laplace spec. That is right for `ctSample()`'s own callers and wrong for two of the three routes the fitting path can take: the marginal routes would have sampled the joint posterior instead, and the state-explicit route would have been refused outright. A worker is now told what to sample rather than which function to be -- where to start, which engine entry, and whether the objective is the state-explicit one -- and rebuilds its own Julia objective from that, since a Julia handle belongs to one session and cannot be sent anywhere. The Hessian travels as the plain matrix it is, so workers metre their chains with the parent's rather than each spending 2n gradients recomputing it. On this path the workers are warmed *before* the optimisation rather than after it, which is what the warming was written for and what no caller was doing; `ctSample()` starts from a fit that is already optimised, has nothing to overlap, and still pays the 26-43 s compile serially.
- The `backend='julia'` bridge was paying a TCP stall, not a latency, on every call R drives one at a time -- `ctJuliaEvaluate()`, `ctKalman()`, `ctPredict()`, the post-fit uncertainty phase, the summary matrices. JuliaConnectoR assembles each message from about ten small writes (an indicator byte, a length, a name, a count, then each argument) and reads the reply the same way, and on Linux that write-write-read pattern meets Nagle's algorithm on the sender and a delayed ACK on the receiver: the second write waits up to 40 ms for an acknowledgement the kernel is deliberately holding back. Measured on a bare socket with neither Julia nor R packages involved, on a 23-core Linux box: one write per request 0.033 ms, two writes 40.7 ms, eight writes 40.8 ms. One stall per message, whatever the payload -- which is why payload size, return shape, return field count and keyword count had all failed to explain anything. A message therefore cost 41 ms if one direction split and 82 ms if both did, and `ctJuliaEvaluate()` sends six of them (marshal the parameter vector; release the proxies R has garbage-collected; evaluate; then three for `juliaGet`, which sets a translation flag, fetches, and unsets it), which is the ~370 ms that had been read as nine round trips of 42 ms each. `ctJuliaSetup()` now turns both halves off: `nagle(io, false)` on the Julia socket is permanent and frees the Julia-to-R direction, and a Julia task re-arms TCP_QUICKACK so the kernel acknowledges R's segments at once, which is what releases R's own Nagle. The task is necessary rather than tidy: R sets no socket options and offers no way to, so the receiving side is the only side that direction can be fixed from; Linux drops quickack again after a few packets; and the stall happens while R is mid-message, where no call from R can reach, but a Julia task can, because it runs precisely when the main task is parked in the socket read. Per bridge message on that box: 81.4 ms untuned, 40.9 ms with Nagle off, 0.8 ms with both. On a 2-latent, 40-subject, 16-parameter Laplace fit whose engine work is 17.8 ms, `ctJuliaEvaluate()` goes from 414 ms to 27.7 ms and `ctKalman()` from 698 ms to 108 ms; on a 4-latent, 120-subject, 48-parameter one, where the engine work is 271 ms, from 666 ms to 266 ms and from 721 ms to 190 ms. The optimiser is unaffected either way, because its loop is inside Julia and crosses the bridge once. Windows and macOS are unaffected too: Windows measures ~0.3 ms a message untuned, and TCP_QUICKACK exists only on Linux, so those platforms get the Nagle half and no task. Nothing numeric moves -- log likelihood and gradient are bit-identical across all three settings on both shapes -- and the engine-only time, which socket options cannot touch, stayed within 1% across every arm while the calls above moved by up to 15x. `options(ctsem.julia.tunebridge = FALSE)` leaves the socket exactly as JuliaConnectoR opened it, and `options(ctsem.julia.quickack = 0)` keeps the Nagle half without the task.
- Still on the table, and now quantified rather than guessed at: three of `ctJuliaEvaluate()`'s six messages are `juliaGet`, which exists only because the engine returns a NamedTuple and JuliaConnectoR sends those by reference. A flat `Array{Float64}` return comes back by value in the call's own message, so a flattened R-facing return would drop those three. That was worth ~220 ms a call before the socket fix and is worth ~5 ms after it, which is why it has not been done. The cleaner fix for the remaining cost belongs upstream: if JuliaConnectoR buffered each message and issued one `send()`, the R-to-Julia stall would not exist and the quickack task would be unnecessary.
