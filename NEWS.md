...
- `ctGenerate(backend='auto')` prefers the julia engine wherever that engine can generate the model, having previously used the R generator for anything neither nonlinear nor categorical. Seed-for-seed output therefore changes for a linear gaussian model with no individually varying parameters on a machine with a julia session; `backend='r'` pins the previous output.
