...
- `plot()`, `ctSubjectPars()`, `ctPredict()`, `ctKalman()`, `ctPredictTIP()`, `ctResiduals()`, `ctACFresiduals()`, `ctDiscretePars()`, `ctSummaryMatrices()`, `ctExtract()`, `ctGenerateFromFit()`, `ctPostPredPlots()` and `ctFitCheckCov()` all accept julia fits.
...
- Individual variation on DIFFUSION or MANIFESTVAR is now estimated as a regression on the individually varying parameters that reach the observation mean, which is the only part of the population covariance that `intoverpop='augmented'` identifies. `poprank=NA` estimates the full covariance as before, and a smaller rank is an explicit approximation. Julia backend only.
- Posterior predictive checking is consolidated into `ctPostPredPlots()`, with clearer plots and an explanatory note on each panel that can be switched off. New panels for the mean trajectory over time, banded for both the spread in the data and the uncertainty in its mean, and for the lagged covariance structure. `ctPostPredict()` remains as an alias, and now accepts julia backend fits.
- Fixed: the observed data and the model were labelled the wrong way round in the bivariate `ctPostPredict()` plots.
- `ctFitCheck()` (`ctCheckFit()`) is removed; its covariance heatmaps produced no output on their default settings, and the diagnostics worth having from it are now panels of `ctPostPredPlots()`. Use that, or `ctFitCheckCov()` for the lagged covariance check on its own.
...
- Fixed: standard errors along a direction the data does not identify were computed by flooring the information matrix and inverting, which produced an arbitrary finite width set by rounding rather than by the data, and could inflate the intervals of well determined parameters alongside it. Such directions are now excluded, parameters lying along them are reported as unidentified rather than given a spread, and `fit$uncertainty$intervalcheck` reports each standard error against the width that parameter's own curvature supports.
...
- `ctGenerate()` gains `fromPriors=TRUE`, which draws the parameters from the model's priors and returns `nsamples` datasets rather than one -- the prior predictive, asked of a model alone. It needs no fitted model and no data: with no `datastruct`, a balanced design of `n.subjects` subjects at `Tpoints` occasions is used. `backend` chooses which engine turns the draws into data. `ctGenerateFromPriors()` is deprecated in its favour; it now draws from the prior directly rather than fitting the model to an empty dataset to approximate it, and a model using `laplaceprior` is refused by name where before it was silently drawn from a normal approximation.
- `ctGenerate(backend='julia')` generates individually varying parameters and time independent predictor effects, where it previously refused a model that asked for them. A population standard deviation is `sdscale` on the parameter -- when generating rather than fitting there is no prior to scale, so the number written there is simply the sd -- a predictor effect is written in the effect itself (`'mypar||||TI1=4.3'`), and a whole population covariance goes in the new `POPCOV` matrix, which wins where both are given. Nothing is drawn from a prior unless `fromPriors=TRUE`, so two generations from one specification agree, and every value supplied for an unstated quantity is named in a message.
- `ctGenerate(backend='auto')` prefers the julia engine for any model given as a `ctModel(type='ct'/'dt')` object, having previously used the R generator for anything neither nonlinear nor categorical. The R generator still takes the matrix-list model form, and stands in wherever no julia session is available. Seed-for-seed output therefore changes on a machine with julia; `backend='r'` pins the previous output.
- Fixed: fitting with `backend='stan'` and `optimcontrol$estonly=TRUE` failed for any model with time independent predictors, which also stopped `ctLOO()` on such models.
