...
- `ctPostPredPlots()` no longer lets an extreme posterior predictive set the axis. Where a variable's model range is many times wider than its data, the axis holds the data and the model's 0.5-99.5% range, and the panel's note says how far the rest reaches; ordinary fits are unchanged. `options(ctsem.plotview = FALSE)` restores the untrimmed axis.
- Posterior predictive checking is consolidated into `ctPostPredPlots()`, with clearer plots and an explanatory note on each panel that can be switched off. New panels for the mean trajectory over time, banded for both the spread in the data and the uncertainty in its mean, and for the lagged covariance structure. 
